...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Transferir la custodia o ubicación de un frasco (US-020)",
)
def registrar_transferencia(
    ...
    """US-020. Transferir la custodia o laboratorio de un frasco.

    Inserta un movimiento de 'TRANSFERENCIA' (cantidad 0 g) en el Kardex.
    El trigger de la base de datos se encarga de actualizar el frasco.
    """
    frasco = connection.execute(
        """
        SELECT f.id_frasco, f.estado, f.id_investigador, f.id_laboratorio_actual,
               inv_orig.nombre AS custodio_anterior,
               lab_orig.nombre AS laboratorio_anterior
          FROM frasco f
          LEFT JOIN investigador inv_orig ON inv_orig.id_investigador = f.id_investigador
          LEFT JOIN laboratorio lab_orig  ON lab_orig.id_laboratorio = f.id_laboratorio_actual
         WHERE f.id_frasco = %s
        """,
        (payload.id_frasco,),
    ).fetchone()
    ...
    if payload.id_investigador_destino is None and payload.id_laboratorio_destino is None:
        raise ProblemException(
            422, "VALIDACION", "Destino no especificado",
            "Debe especificar al menos un nuevo custodio o laboratorio de destino.",
            field="id_investigador_destino",
        )

    custodio_nuevo = None
    if payload.id_investigador_destino is not None:
        inv_dest = connection.execute(
            "SELECT nombre FROM investigador WHERE id_investigador = %s",
            (payload.id_investigador_destino,),
        ).fetchone()
        if inv_dest is None:
            raise ProblemException(
                404, "INVESTIGADOR_NO_EXISTE", "Investigador inexistente",
                f"No existe el investigador {payload.id_investigador_destino}.",
                field="id_investigador_destino",
            )
        custodio_nuevo = inv_dest["nombre"]

    laboratorio_nuevo = None
    if payload.id_laboratorio_destino is not None:
        lab_dest = connection.execute(
            "SELECT nombre FROM laboratorio WHERE id_laboratorio = %s",
            (payload.id_laboratorio_destino,),
        ).fetchone()
        if lab_dest is None:
            raise ProblemException(
                404, "LABORATORIO_NO_EXISTE", "Laboratorio inexistente",
                f"No existe el laboratorio {payload.id_laboratorio_destino}.",
                field="id_laboratorio_destino",
            )
        laboratorio_nuevo = lab_dest["nombre"]
    ...
    fila = connection.execute(
        ...
        INSERT INTO kardex (
          id_frasco, tipo_movimiento, motivo, cantidad_g,
          id_investigador_origen, id_investigador_destinatario,
          id_laboratorio_origen, id_laboratorio_destino,
          fecha_operacion, curso, registrado_por, saldo_resultante_g
        ) VALUES (
          %s, 'TRANSFERENCIA', 'transferencia', 0,
          %s, %s,
          %s, %s,
          COALESCE(%s, CURRENT_DATE), %s, %s, 0
        )
        RETURNING id_movimiento, fecha_hora
        """,
        (
            payload.id_frasco,
            frasco["id_investigador"], payload.id_investigador_destino,
            frasco["id_laboratorio_actual"], payload.id_laboratorio_destino,
            payload.fecha_operacion, payload.observaciones or "Transferencia de custodia",
            user.id_usuario,
        ),
    ).fetchone()
    ...
    return TransferenciaOut(
        ...
        id_investigador_origen=frasco["id_investigador"],
        id_investigador_destinatario=payload.id_investigador_destino,
        custodio_anterior=frasco["custodio_anterior"],
        custodio_nuevo=custodio_nuevo or frasco["custodio_anterior"],
        id_laboratorio_origen=frasco["id_laboratorio_actual"],
        id_laboratorio_destino=payload.id_laboratorio_destino,
        laboratorio_anterior=frasco["laboratorio_anterior"],
